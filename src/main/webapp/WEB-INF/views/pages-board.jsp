...
<body>
	...
	<%@include file="../views/member.jsp" %>
			...
			<main class="content">
				...
				<form action="bList" method="get" style="float:right">
	...
	<c:if test="${paging.page > 1}"><a href="bList?page=${paging.page-1}&limit=${paging.limit}" class="btn btn-secondary" >〈이전</a></c:if>
	...
	<c:forEach var="b" begin="${paging.startPage}" end="${paging.endPage}" step="1">
		<c:choose>
			...
			<c:otherwise>
				<a href="bList?page=${b}&limit=${paging.limit}" class="btn btn-secondary">${b}</a>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<a href="bList?page=${paging.page+1}&limit=${paging.limit}" class="btn btn-secondary"> 다음〉</a>
								...
								<button onclick="bWrite()" style="float:right" class="btn btn-secondary">글쓰기</button>
								</div>
								</div>
								<br/><br/>
