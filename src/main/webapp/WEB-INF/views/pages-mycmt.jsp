...
<body>
	...
	<%@include file="../views/member.jsp" %>
			...
			<main class="content">
				...
				<form action="myCmt" method="get" style="float:right">
			...
			<input type="hidden" name="mId" value="${paging.MId}">
							...
							<table class="table table-hover my-0">
								<thead>
									<tr>
										...
										<th class="d-none d-xl-table-cell">댓글</th>
								...
								<tbody>
									<c:forEach var="list" items="${myCmt}">
										<tr>
											<td class="d-none d-xl-table-cell"><a href="bView?bNum=${list.cbNum}">${list.cbNum}</a></td>
											<td class="d-none d-xl-table-cell">${list.cmtWriter}</td>
											<td class="d-none d-xl-table-cell">${list.cmtContent}</td>
											<td class="d-none d-xl-table-cell">${list.cmtDate}</td>
										</tr>
									</c:forEach>
	...
	<c:if test="${paging.page > 1}"><a href="myCmt?mId=${paging.MId}&page=${paging.page-1}&limit=${paging.limit}" class="btn btn-secondary" >〈이전</a></c:if>
	...
	<c:forEach var="b" begin="${paging.startPage}" end="${paging.endPage}" step="1">
		<c:choose>
			...
			<c:otherwise>
				<a href="myCmt?mId=${paging.MId}&page=${b}&limit=${paging.limit}" class="btn btn-secondary">${b}</a>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<a href="myCmt?mId=${paging.MId}&page=${paging.page+1}&limit=${paging.limit}" class="btn btn-secondary"> 다음〉</a>
