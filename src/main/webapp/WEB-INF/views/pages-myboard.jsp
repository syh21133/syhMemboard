...
<body>
	...
	<%@include file="../views/member.jsp" %>
			...
			<main class="content">
				...
				<form action="mybList" method="get" style="float:right">
	...
	<c:if test="${paging.page > 1}"><a href="mybList?mId=${paging.MId}&page=${paging.page-1}&limit=${paging.limit}" class="btn btn-secondary" >〈이전</a></c:if>
	...
	<c:forEach var="b" begin="${paging.startPage}" end="${paging.endPage}" step="1">
		<c:choose>
			...
			<c:otherwise>
				<a href="mybList?mId=${paging.MId}&page=${b}&limit=${paging.limit}" class="btn btn-secondary">${b}</a>
			</c:otherwise>
		</c:choose>
	</c:forEach>
	...
	<a href="mybList?mId=${paging.MId}&page=${paging.page+1}&limit=${paging.limit}" class="btn btn-secondary"> 다음〉</a>
